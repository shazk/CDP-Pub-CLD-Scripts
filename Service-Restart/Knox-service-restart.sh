...
for CLUSTER_SERIVCE_NAME in $(curl -s -L -k -u ${WORKLOAD_USER}:${WORKLOAD_USER_PASS} -X GET "${CM_SERVER}/api/${CM_API_VERSION}/clusters/${CM_CLUSTER_NAME}/services/" | awk -F "[ |:|,]" '/name.*knox/ {print $(NF - 1 )}' | sed 's|"||g')
do
  ...
  select ANSWER in "Start" "Stop" "Status" "Restart" "Exit"
  do
    case ${ANSWER} in
    "Start")
